...
import { Button, ExternalLinkIcon, FlytrapIcon } from "@flytrap/ui";
...
const heroStrip = [
  ["What's new", "v1.0.0"],
  ["Design principles", "Nossa filosofia"],
  ["Contribute", "Entre no ecossistema"],
...
export function Hero() {
  return <section aria-labelledby="living-hero-title" className="relative isolate overflow-hidden border-b border-white/10 px-6 pb-10 pt-14 text-white md:px-10 lg:min-h-[92vh]" id="overview">
    ...
    <CharacterLayer
      ...
      className="absolute right-[-16vw] top-[2%] z-0 hidden h-[min(100vh,1040px)] w-[min(100vh,1040px)] sm:block lg:right-[-7vw]"
      pose="standing"
    />

    <div className="relative z-10 max-w-xl">
      <p className="font-mono text-xs uppercase tracking-[0.34em] text-[#ff4fbd]">Design System</p>
      <h1 className="mt-5 font-display text-5xl font-bold leading-[1.04] tracking-tight md:text-6xl xl:text-7xl" id="living-hero-title">
        A living language
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        <Button asChild className="bg-[#F10081] text-white shadow-[0_0_28px_rgba(241,0,129,.45)] hover:bg-[#CF006A]" size="lg">
          <a href="#foundations">Get started <span aria-hidden="true">→</span></a>
        </Button>
        <Button asChild className="border-white/25 bg-white/5 text-white hover:bg-white/10" size="lg" variant="outline">
    ...
    <div className="relative z-10 mt-10 hidden w-64 flex-col gap-4 xl:absolute xl:right-10 xl:top-16 xl:flex">
      ...
      <FloatingPanel delay={-2.4} title="Bio-signal feed">
        <div className="flex items-center justify-between">
          <div aria-hidden="true" className="flytrap-motion flytrap-organic size-16 animate-[flytrap-pulse_4.6s_ease-in-out_infinite] bg-[radial-gradient(circle_at_40%_40%,rgba(255,79,189,.85),rgba(241,0,129,.35)_60%,transparent_78%)] blur-[1px]" />
    ...
    <div className="relative z-10 mt-12 grid gap-px overflow-hidden rounded-2xl border border-white/10 bg-white/8 backdrop-blur-xl sm:grid-cols-2 lg:mt-16 lg:max-w-4xl xl:grid-cols-4">
      {heroStrip.map(([title, detail], index) => <div className="flex items-center gap-3 bg-[#0a0c14]/85 px-4 py-3" key={title}>
        {index === heroStrip.length - 1 && <span className="size-2 rounded-full bg-[#b8ff35] shadow-[0_0_10px_rgba(184,255,53,.9)]" />}
        <span>
