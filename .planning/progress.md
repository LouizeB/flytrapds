...
- [x] Expandir JSDoc para componentes prioritários do Memory Chat: `Input`, `Form`, `Select`, `Dialog`, `ChatThread`, `PromptInput` e `RecommendationRail` agora geram chunks de prop descriptions para perguntas sobre estados, acessibilidade, submissão, empty/error/loading e recomendações; o relatório agora mostra 101 fontes, 501 IDs, 394 chunks gerados, 0 duplicados e backlog direto zerado.
- [x] Expandir JSDoc para navegação e overlays: `DropdownMenu`, `Combobox`, `CommandMenu`, `Tabs`, `Popover`, `Sheet` e `Tooltip` agora geram chunks de prop descriptions para perguntas sobre teclado, foco, listbox, disabled, checked/indeterminate, sideOffset, alinhamento, portal e conteúdo não essencial; o relatório agora mostra 101 fontes, 508 IDs, 401 chunks gerados, 0 duplicados e backlog direto zerado.
- [x] Simplificar a comunicação inicial de docs, Studio e Dashboard em inglês, reduzindo jargão e exibindo uma área de trabalho por vez nos consumidores.
- [x] Fixar Studio e Dashboard em dark mode e corrigir a geração de aliases derivados para que sidebar, cards, KPIs e demais componentes resolvam tokens dentro do modo ativo.
