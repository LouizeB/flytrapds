...
export const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitive.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitive.Close>>(({ className, ...props }, ref) => <ToastPrimitive.Close ref={ref} aria-label="Fechar notificação" className={cn("absolute right-2 top-2 rounded-md p-1 text-muted-foreground outline-none opacity-70 hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 focus-visible:ring-ring", className)} {...props}><FlytrapIcon icon={CloseIcon} size="sm" /></ToastPrimitive.Close>);
