...
export function ChatThread({ state = "ready", emptyTitle = "Comece uma conversa", emptyDescription = "Envie uma mensagem para iniciar.", emptyAction, errorMessage = "Não foi possível carregar a conversa.", className, children, ...props }: ChatThreadProps) {
  return <div aria-busy={state === "loading" || undefined} className={cn("grid min-h-48 content-start gap-3 overflow-y-auto rounded-xl border bg-background p-4", className)} data-slot="chat-thread" data-state={state} role="log" {...props}>
    ...
    {state === "loading" && <div aria-label="Carregando conversa" className="grid gap-3"><Skeleton className="h-16 w-3/4" /><Skeleton className="ml-auto h-12 w-2/3" /><Skeleton className="h-20 w-4/5" /></div>}
