...
`memory:generate` rebuilds generated semantic chunks from documentation and component metadata. `memory:report` runs generation first, scans documentation and UI/AI/chart component files, compares them with the indexed memory sources and writes `.planning/memory-index-candidates.md`.
